...
class OllamaOnlyEmbeddingFunction(EmbeddingFunction[Documents]):
    """Prevent Chroma from selecting its built-in ONNX embedding model.

    Embeddings are generated asynchronously below through the locally configured
    Ollama model and are always supplied to Chroma explicitly.
    """

    def __call__(self, input: Documents) -> Embeddings:
        raise RuntimeError("RefinaAI embeddings must be supplied by Ollama; Chroma default embeddings are disabled.")
...
collection = chroma_client.get_or_create_collection(
    ...
    embedding_function=OllamaOnlyEmbeddingFunction(),
)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        for text in texts:
            try:
                ...
                raise RuntimeError(
                    "Ollama embedding service is unavailable. Start Ollama and verify 127.0.0.1:11434."
                ) from exc
            except (ValueError, KeyError) as exc:
                raise RuntimeError(f"Embedding model '{EMBED_MODEL}' returned an invalid response.") from exc
    return embeddings
...
async def query_rag(query: str, n_results: int = 4) -> list[dict]:
    ...
    results = collection.query(query_embeddings=[query_vec], n_results=min(max(n_results, 1), document_count))
    hits = []
